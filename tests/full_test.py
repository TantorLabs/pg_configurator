...
class UnitTestProfiles(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    def test_02_profiles_1c(self):
        ...
        def tst_1c(ver, sample):
            ...
            print(str(json.dumps(results, indent=4)))
            self.assertTrue(results[ver] == sample)
    ...
    def test_03_platform_profiles_1c(self):

        def tst_platf_1c(ver, sample):
            parser = PGConfigurator.get_arg_parser()
            results = {}
            testname = self._testMethodName
            outputfile = '%s/%s-%s.conf' % (params.output_dir, testname, ver)
            args = parser.parse_args([
                '--output-file-name=%s' % outputfile,
                '--conf-profiles=profile_platform_perf,profile_1c',
                '--pg-version=%s' % ver,
                '--db-cpu=8',
                '--db-ram=2048Mi',
                '--db-disk-type=SAS',
        ...
        sample = json.loads("""
        {
            ...
            "16": {
                ...
                "wal_writer_flush_after": "1764kB",
                "work_mem": "10000kB",
                "transaction_buffers": "15MB"
            }
        }
        """)

        tst_platf_1c("14", sample["14"])
        tst_platf_1c("15", sample["15"])
        tst_platf_1c("16", sample["16"])
